...
export default {
  ...
  methods: {
    judgeHelp() {
      ...
      if (router == "all") {
        this.axios
          .get(`/xuptbbs/project?&page=${this.page}&per_page=10`)
          .then(res => {
        ...
        this.axios
          .get(`/xuptbbs/project/type?id=0&page=${this.page}&per_page=10`)
          .then(res => {
            this.itemlist = res.data.content;
            console.log(res);
          });
        ...
        this.axios
          .get(`/xuptbbs/project/type?id=1&page=${this.page}&per_page=10`)
          .then(res => {
            this.itemlist = res.data.content;
            console.log(res);
          });
  ...
  filters: {
    formatCard(key) {
      var card = [{ key: "0", value: "无偿" }, { key: "1", value: "有偿" }];
    ...
    formatType(key) {
      var card = [{ key: "0", value: "找队友" }, { key: "1", value: "找项目" }];
      for (var i = 0; i < card.length; i++) {
        if (card[i].key == key) {
          return card[i].value;
        }
    ...
    classCard(key) {
      var card = [{ key: "0", value: "bl" }, { key: "1", value: "re" }];
      for (var i = 0; i < card.length; i++) {
        if (card[i].key == key) {
          return card[i].value;
        }
